...
from train_data.toy_data.main_2D import plot_muller_train_data
from train_data.toy_data.main_2D import generate_data_and_save
...
def check_and_generate_data(paths, data_type):
...
def read_data(paths, data_type):
    ...
    check_and_generate_data(paths, data_type)

    all_data = []
    all_labels = []

    pt_files = [os.path.join(paths, f) for f in os.listdir(paths) if f.endswith('.pt')]

    for i, file_path in enumerate(pt_files):
        adata = torch.load(file_path)
        adata = adata.reshape(-1, 1, 66)
    ...
    if data_type == 'ADC':
        combined_data1 = combined_data * 10   # Convert units nm to 0.1 angstrom
    elif data_type == 'ADI':
