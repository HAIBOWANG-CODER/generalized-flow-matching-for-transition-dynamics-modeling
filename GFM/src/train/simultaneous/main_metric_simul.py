...
def run_processing_for_pdb(files0, files1, velocity_net, flow_matcher_base, traj_path_prefix, data_type, t_steps=501, num=0):
    """
            ...
            if reflow:
                ...
                if n % 1 == 0 or converge:
                    if direc == "bidirectional":
                        # Sample a new batch for bidirectional training
                        ...
                        # Unidirectional case: use x1 directly
                        x1T = x1_batch
                    else:
                        raise ValueError(f"Don't know the direction")

                else:
                    # Neural ODE trajectory computation for reflow training
                    with torch.no_grad():
                        node = NeuralODE(
                            velocity_net,
                            solver="euler",
