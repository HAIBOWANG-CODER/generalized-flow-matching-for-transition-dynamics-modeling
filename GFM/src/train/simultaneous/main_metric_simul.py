...
def process_trajectory(flow_net, flow_matcher_base, x0, x1, t_steps, traj_path_prefix, num, data_type):
    ...
    torch.save(xt.detach(), f'{traj_path_prefix}/bendnet_tensor_{num}.pt')
    ...
    for epoch in tqdm(range(num_epochs), desc=desc):
        ...
        for x0_batch, x1_batch in zip(x0_loader, x1_loader):
            ...
            if reflow:
                ...
                x0_batch = x0_batch.to(next(bend_model.parameters()).device)
                x1_batch = x1_batch.to(next(bend_model.parameters()).device)
            ...
            if direc == "bidirectional":
                # Spline
                loss0 = bend_model._compute_loss([[x0, indice], [x1T, indice]], [x0, x1], exp_G=True)
                loss1 = bend_model._compute_loss([[x0T, indice], [x1, indice]], [x0, x1], exp_G=True)
                spline_loss = loss0 + loss1
                # Velocity
                ts0, xts0, uts0 = bend_model._process_flow([x0], [x1T], t, exp_G=True)
                ts1, xts1, uts1 = bend_model._process_flow([x0T], [x1], t, exp_G=True)
                ...
                # Spline
                if resample:
                    loss0 = bend_model._compute_loss([[x0, indice], [x1T, indice]], [x0, x1],
...
optimizer = Adam([
    {'params': bend_net.parameters(), 'lr': args.spline_lr},
