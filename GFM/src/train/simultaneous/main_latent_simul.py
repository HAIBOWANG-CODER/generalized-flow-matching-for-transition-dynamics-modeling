...
def train_gfm(spline, velocity_net, x0_loader, x1_loader, optimizer, num_epochs):
    spline.train()
...
train_gfm(spline, velocity_net, x0_loader, x1_loader, optimizer, num_epochs=epochs)
...
traj = traj.detach()
if data_type == 'ADC':
    xts = traj / 100        # Convert units to nanometers
elif data_type == 'ADI':
    # Transform internal coords to Cartesian coords
    xts = internal_to_cartesian(traj)
else:
