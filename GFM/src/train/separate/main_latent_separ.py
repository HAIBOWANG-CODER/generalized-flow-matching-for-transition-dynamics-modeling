...
def train_flow(x0_loader, x1_loader, spline, flow_net, ot_sampler, flow_optimizer, num_epochs, batch_size):

    for epoch in tqdm(range(num_epochs), desc="Flow Training Epochs"):
        total_loss_epoch = 0
        ...
        wandb.log({
            "FlowNet/flow_total_loss_epoch": avg_loss,
        })
...
train_flow(x0_loader, x1_loader, spline, flow_net, ot_sampler, flow_optimizer, num_epochs=flow_epochs, batch_size=batch_size)
...
torch.save(xts, f'{args.save_address}/flownet_tensor.pt')
