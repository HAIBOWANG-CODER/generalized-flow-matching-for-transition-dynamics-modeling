...
class sampleLoader:
    """
         A loader class to load and preprocess molecular coordinate data from a file.

         Args:
             path (str): Path to the file containing coordinate data.
             data_type (str): Type of data, e.g., 'ADC' (Alanine Dipeptide Cartesian) or 'ADI' (Alanine Dipeptide Internal).
             max_atoms (int): Maximum number of atoms expected in the data. Default is 66.
    """
    ...
    def load_all_data(self):
        ...
        elif self.data_type == 'muller':        # Muller Brown potential data
...
class SplineMLP(nn.Module):
    def __init__(self, input_dim: int, activation: str, batch_norm: bool = True,
                 hidden_dims: Optional[List[int]] = None, time_spline: bool = False, time_embedding_type: str = "cat",
...
def process_trajectory(velocity_net, flow_matcher_base, x0, x1, t_steps, traj_path_prefix, num, data_type):
    ...
    xt = flow_matcher_base.sample_location_and_conditional_flow(x0, x1, 0, 1, t=t, spline_test=True)
...
def run_processing_for_pdb(files0, files1, velocity_net, flow_matcher_base, traj_path_prefix, data_type, t_steps=501,
                           num=0):
    """
...
def train_spline(
    ...
    for epoch in tqdm(range(num_epochs), desc=desc):
        ...
        for x0_batch, x1_batch in zip(x0_loader, x1_loader):
            ...
            if reflow:
                # Reflow process
                x0 = x0_batch[:, 0, :]         # Forward data
                x1T = x0_batch[:, 1, :]

                x0T = x1_batch[:, 0, :]          # Backward data
                x1 = x1_batch[:, 1, :]
            else:
            ...
            if direc == "bidirectional":
                # Compute spline loss for both forward and backward directions
                loss0 = spline_model._compute_loss([x0, x1T])
                loss1 = spline_model._compute_loss([x0T, x1])
                spline_loss = loss0 + loss1

            elif direc == "unbidirectional":
                # Compute spline loss for unidirectional case
                if resample:
                    # Resample process
...
def train_velocity(x0s, x1s, flow_matcher_base, velocity_net, num_epochs, optimizer, reflow=False, direc="unbidirectional"):
    for epoch in tqdm(range(num_epochs), desc=f"Train velocity net. Now epoch/Epochs:"):
        ...
        for x0_batch, x1_batch in zip(x0s, x1s):
            ...
            if reflow:
                ...
                x0 = x0_batch.to(next(velocity_net.parameters()).device)
                x1T = x1_batch.to(next(velocity_net.parameters()).device)
            ...
            if direc == "bidirectional":
                # Compute velocity loss for unidirectional case
                if reflow:
                    x0T = x1_batch[:, 0, :]
                    x1 = x1_batch[:, 1, :]
                else:
...
if args.data_type in ('ADC', 'ADI'):
    ...
    train_velocity(x0_loader, x1_loader, flow_matcher_base, velocity_net, epochs, optimizer_velocity, direc=direc)
    ...
    if reflow:
        ...
        for i in range(reflow_num):
            ...
            train_velocity(dataloader_forward, dataloader_backward, flow_matcher_base,
                           velocity_net, epochs, optimizer_velocity, reflow=True, direc=direc)
    ...
    if resample:
        ...
        for j in range(resample_num):
            ...
            train_velocity(x0_loader, x1_loader, flow_matcher_base, velocity_net, epochs, optimizer_velocity)
    ...
    x0 = torch.tensor(loader0.load_all_data(), dtype=torch.float32)
    """
    loader1 = sampleLoader(toy_fles1, data_type, max_atoms=args.dim)
    x1 = torch.tensor(loader1.load_all_data(), dtype=torch.float32)

    t = torch.linspace(0, 1, steps=501)
    tra = flow_matcher_base.sample_location_and_conditional_flow(
        x0, x1, 0, 1, t=t, spline_test=True
    )
    torch.save(tra.detach(), r"~/GFM/traj/spline_x.pt")
