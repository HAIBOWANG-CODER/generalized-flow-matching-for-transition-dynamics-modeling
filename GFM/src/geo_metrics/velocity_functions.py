...
class VelocityOnManifold:
    ...
    def calculate_metric(self, x_t, current_timestep):
        """
            Calculates a metric for the given input `x_t` at the specified `current_timestep`.

            This method initializes and trains Radial Basis Function (RBF) networks if called for the first time,
            or loads a pre-trained RBF network if in "reflow" mode. It then computes the metric using the RBF network.
        """
        M_dd_x_t = None
