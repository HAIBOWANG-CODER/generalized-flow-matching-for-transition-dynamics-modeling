...
from src.geo_metrics.land import land_metric_tensor
    ...
    def calculate_metric(self, x_t, samples, current_timestep):
        ...
        if self.metric == "land":
            ...
            if self.called_first_time:
                ...
                for timestep in range(self.datamodule.num_timesteps - 1):
                    ...
                    if self.image_hx:
                        ...
                        self.rbf_networks.append(rbf_network)
                        torch.save(rbf_network.state_dict(), f"RBF_{self.metric}.pth")
            ...
            if self.now_reflow != 0:
                ...
                self.rbf_networks = []
                rbf_network.load_state_dict(torch.load(f"RBF_{self.metric}.pth"))
                self.rbf_networks.append(rbf_network)
