...
def weighting_function(x, samples, gamma):
    # x should have shape [B, D] and samples should have shape [N, D]
    # We calculate the pairwise squared differences for each x_t in the batch against all samples
    # Utilize broadcasting: x[:, None, :] - samples[None, :, :] results in shape [B, N, D]
