...
def internal_to_cartesian(inter_data):
    ...
    inter_coords = inter_data.reshape(-1, 66).to(device)
    ...
    if inter_data.dim() == 3:
        cart_tensor = cart_tensor.reshape(inter_data.shape[0], -1, 66)
    elif inter_data.dim() == 2:
        cart_tensor = cart_tensor.reshape(-1, 66)
    else:
...
def cartesian_to_internal(cart_data):
    ...
    cart_coords = cart_data.reshape(-1, 66)
    ...
    if cart_data.dim() == 3:
        cart_tensor = cart_tensor.reshape(cart_data.shape[0], -1, 66)
...
if __name__ == "__main__":
    ...
    torch.save(internal_tensor, r"~/pdb_data/internal_data/x0s.pt")
    """
    # ================================ internal to cartesian ===================================== #
    # === #
    internal_path = r"~\traj\linear_internal.pt"                          # Internal coordinate data path
    ...
    torch.save(cartesian_tensor, r"~\traj\internal.pt")                     # Output
